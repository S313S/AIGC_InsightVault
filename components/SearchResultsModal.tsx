...
export const SearchResultsModal: React.FC<SearchResultsModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-0 sm:p-4">
            <div className="bg-[#0d1526]/95 backdrop-blur-xl rounded-none sm:rounded-2xl w-full h-[100dvh] sm:h-auto sm:max-h-[85vh] sm:max-w-4xl flex flex-col shadow-2xl border border-[#1e3a5f]/50">
                {/* Header */}
                <div className="p-4 sm:p-6 border-b border-[#1e3a5f]/40 flex items-start sm:items-center justify-between gap-3">
                        ...
                        <div className="space-y-3">
                            {results.map(result => (
                                <div
                                    ...
                                    className={`flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl border cursor-pointer transition-all ${selectedIds.has(result.noteId)
                                ...
                                >
                                    <div className="flex items-start gap-3 min-w-0 flex-1">
                                        {/* Checkbox */}
                                        <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 mt-0.5 ${selectedIds.has(result.noteId)
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-semibold text-gray-100 break-words line-clamp-2">
                ...
                {!isLoading && results.length > 0 && (
                    <div className="p-3 sm:p-4 border-t border-[#1e3a5f]/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <button
                            onClick={selectAll}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
