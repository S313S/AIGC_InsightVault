...
import { LayoutGrid, Plus, Search, Database, Menu, MessageSquare, Activity, Home, Folder, X, Check, MoreVertical, Edit2, Trash2, CheckSquare, FolderPlus, Sparkles } from './components/Icons';
...
const App: React.FC = () => {
  const [cards, setCards] = useState<KnowledgeCard[]>(INITIAL_DATA);
  const [tasks, setTasks] = useState<TrackingTask[]>(INITIAL_TASKS);
  const [trending, setTrending] = useState<KnowledgeCard[]>(TRENDING_DATA);
  ...
  const handleAddCard = (newCard: KnowledgeCard) => {
    setCards(prev => [newCard, ...prev]);
  };

  const handleAddTask = (newTask: TrackingTask) => {
    setTasks(prev => [newTask, ...prev]);
  };

  const handleSaveTrendingToVault = (card: KnowledgeCard) => {
      // Add to main cards list
      setCards(prev => [{...card, id: Date.now().toString()}, ...prev]);
  ...
  const handleRenameCollection = (e: React.MouseEvent, collectionId: string, currentName: string) => {
      e.stopPropagation();
      ...
      if (window.confirm(`Remove ${selectedCardIds.size} items from this collection?`)) {
          setCards(prevCards => prevCards.map(card => {
              if (selectedCardIds.has(card.id)) {
                  // Remove currentCollectionId from the card's collections array
                  return {
                      ...card,
  ...
  const handleBatchAddToCollection = (targetCollectionId: string) => {
      setCards(prevCards => prevCards.map(card => {
          if (selectedCardIds.has(card.id)) {
              // Add targetCollectionId to collections if not already present
              const currentCollections = card.collections || [];
              if (!currentCollections.includes(targetCollectionId)) {
                  return { ...card, collections: [...currentCollections, targetCollectionId] };
              }
          }
          return card;
      }));
